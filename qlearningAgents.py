...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        """
            ...
            actions. Note that if there are no legal actions, which is the
    ...
    def getAction(self, state):
        """
            ...
            self.epsilon, we should take a random action and take the best
            policy action otherwise. Note that if there are no legal actions,
            which is the case at the terminal state, you should choose None as
            the action.
        ...
        i = 0
        while True:
            random_action = legalActions[i]
            if util.flipCoin(self.epsilon):
                break
            i += 1
            i %= numLegalActions
        action = self.getPolicy(random_action)
        return action
...
class PacmanQAgent(QLearningAgent):
    ...
    def getAction(self, state):
        """
            Simply calls the getAction method of QLearningAgent and then informs
            parent of action for Pacman. Do not change or remove this method.
        """
